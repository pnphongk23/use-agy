# Official Herdr Skill Snapshot

This file vendors the official Herdr agent skill so `use-agy` can use the same CLI model without depending on network access at runtime.

## Contents

- [Source and revision](#official-herdr-skill-snapshot)
- [Adapter rules for `use-agy`](#adapter-rules-for-use-agy)
- [Self-update procedure](#self-update-procedure)
- [Vendored upstream content](#vendored-upstream-content)

- Upstream page: https://github.com/ogulcancelik/herdr/blob/master/SKILL.md
- Upstream raw: https://raw.githubusercontent.com/ogulcancelik/herdr/master/SKILL.md
- Reviewed commit: https://github.com/ogulcancelik/herdr/commit/c76e96878b866bf01639c8a1d8beb9c93a8ab95f
- Snapshot commit: `c76e96878b866bf01639c8a1d8beb9c93a8ab95f`
- Snapshot date: `2026-07-20`
- License: AGPL-3.0-or-later; see https://github.com/ogulcancelik/herdr/blob/master/LICENSE

## Adapter Rules For `use-agy`

The snapshot below is upstream material, not a second authority layer. Apply it with these local rules:

1. The upstream `HERDR_ENV=1` gate applies to the standalone skill operating from inside Herdr. The external-supervisor exception exists only when the user explicitly invokes `$use-agy` or explicitly requests Herdr control, and the helper additionally receives `prepare --herdr-authorized`. A mere AGY mention does not qualify. Even then, `scripts/orchestrate_herdr.py` may control only a verified server through a pinned socket and run-owned IDs; the exception never authorizes ad-hoc control of the user's focused session.
2. The installed `herdr` binary remains the syntax authority. Use non-mutating command-group help, parse JSON responses, and treat every ID as opaque.
3. The external supervisor has no caller pane to split safely, so the helper deliberately creates one run-owned workspace with `--no-focus`, parses the returned workspace/tab/bootstrap-pane IDs, starts AGY by those IDs, verifies and closes only the bootstrap pane, and checks that the sole owned pane fills the workspace. This is the local topology exception to upstream's sibling-pane default.
4. Use explicit IDs from the run manifest for every pane operation; never use name or focus as ownership. Close only resources recorded as created by the run, and refuse cleanup when an unexpected pane appears.
5. Treat `idle` and `done` as attention states, not completion evidence. Terminal reads are rendered diagnostics: `recent-unwrapped` repairs soft wraps only and cannot reconstruct child-TUI hard wrapping, replacement, or truncation. The helper's primary completion evidence is the exact raw response read-only from the AGY conversation database identified by the owned job log.
6. The local work order, authority boundary, structured handoff contract, evidence capture, independent verification, and cleanup rules in `SKILL.md` and `herdr-runtime.md` take precedence where they are stricter.

## Self-update Procedure

When refreshing this snapshot:

1. Open the upstream page and inspect its latest commit and repository license.
2. Fetch the raw file at the exact reviewed commit, not only the moving `master` URL.
3. Diff it against the `Vendored Upstream Content` block below.
4. Review behavior changes against the adapter rules. Never automatically overwrite local authority, ownership, evidence, or cleanup policy.
5. Replace the vendored block, then update `Reviewed commit`, `Snapshot commit`, and `Snapshot date` above.
6. Run the `use-agy` orchestration tests and `git diff --check` before accepting the refresh.

## Vendored Upstream Content

The following block is an exact snapshot of upstream `SKILL.md` at the pinned commit above.
